...
class CreateAccountPage extends BasePage {
  ...
  async selectRegionState(regionState) {
    await this.regionStateDropdown.selectOption(regionState);
  }
  ...
  async selectDynamicSubscribeRadioButton(label) {
    const radioButton = this.dynamicSubscribeRadioButtonByLabel(label);
    await this.selectRadioButton(radioButton);
  }

  async selectPrivacyPolicyCheckbox() {
    await this.selectCheckbox(this.privacyPolicyCheckbox);
  }
